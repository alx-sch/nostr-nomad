...
def count_dirs(paths: Paths):
	...
	for path in os.listdir(paths.input_std_dir): 
		...
		if (os.path.isdir(full_path) or path.endswith('.zip')): # Only accepts ONE directory or zip-file.
			count += 1
...
def store_dir(paths: Paths):
	...
	if not (csv_found and posts_found and html_found): # If not only ONE csv-file and a posts-folder with html-file(s), return error.
		error_and_exit(MISSING_SUBSTACK_DATA)
