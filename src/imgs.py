...
import pillow_avif  # noqa: F401 # Pillow plugin for AVIF support, keep it

# local imports
from models import Post, Paths, User
from imgs_uploader import upload_image
from utils import sha256_hash, load_cache, save_cache, print_yellow, ITALIC, RESET, safe_urlopen
...
def replace_img_urls(post: Post, new_url: str, index: int):
    ...
    if 0 <= index < len(img_tags):  # If index is within img_tags range.
        img = img_tags[
            index
        ]  # Picks the right image - corresponding to the correct URL.
...
def download_images(
    paths: Paths, post: Post, user: User, url: str, i: int, cache: dict
):
    """
    ...
    try:
        ...
        if user.image_host == "imgur":
            print_yellow(
                f"Warning: Image format not supported for rehosting: {filename}. Using original URL.\n"
            )
    ...
    if user.image_host == "imgur":
        if orig_format.upper() not in [
            "JPEG",
            "PNG",
            "GIF",
        ]:  # Imgur does not support AVIF, WEBP, ...
...
def extract_image_urls(paths: Paths, post: Post):
    ...
    if post.html_content is None:
        raise ValueError("soup cannot be None")
    img_tags = post.html_content.find_all(
        "img"
...
def handle_imgs(paths: Paths, post: Post, user: User, counts: list[int]):
    ...
    for i, url in enumerate(urls):
        filepath, downloaded, uploadable = download_images(
            paths, post, user, url, i, cache
        )
        ...
        if new_url:
            replace_img_urls(
                post, new_url, i
            )  # Replaces the old image url of the content to point to the new URL.
            post.new_image_urls.append(
                new_url
            )  # Adds to the list of the new image urls.
